import React from "react";

const IndustryCollaborations = () => {
   return(
    <div className="tl-event-details-container pt-50 pb-50 tl-10-about-section" >
        <div className="container">
            <div className="row tl-event-details-row g-0">
                <div className="col-lg-12">
                    <h2 className="universityobject-heading">Academic Partners & Industry Collaborations</h2>
                    <div className="tl-event-details-left">
                        <div className="tl-event-details-key-content tl-event-details-area">
                            <p>The Sanjay Ghodawat University has established MOUs and functional relations with many Institutions and Industries with agreement of supporting student exchange, exchange of scholarly ideas / expertise and research, the support of specific discipline interaction; the development of programs to include student exchange.</p>
                            <h4>Competency Center Being Established For Conducting Practicals</h4>
                               <ul className="tl-event-details-key-content-list phd-container">
                                    <li>Technology Center</li>
                                    <li>Advanced Learning Center</li>
                                    <li>Innovation Center</li>
                                    <li>Teardown and Benchmarking Center</li>
                                    <li>Visualization Center</li>
                                    <li>Advanced Manufacturing Center/Robotics</li>
                                </ul>
                            </div>
                        </div>

                        <div className="tl-event-details-left mt-30">
                        <div className="tl-event-details-key-content tl-event-details-area">
                            <h4>Collaborations with Professional Bodies</h4>
                               <ul className="tl-event-details-key-content-list phd-container">
                                    <li>American Concrete Institute (ACI)</li>
                                    <li>National Remote Sensing Centre (NRSC)</li>
                                    <li>CSI</li>
                                    <li>Indian Society for Technical Education (ISTE)</li>
                                    <li>Indian Geotech Society (IGS)</li>
                                    <li>IUCEE</li>
                                    <li>IARA</li>
                                    <li>ISDSI</li>
                                </ul>
                            </div>
                        </div>

                        <div className="tl-event-details-left mt-30">
                        <div className="tl-event-details-key-content tl-event-details-area">
                            <h4>Collaborations with Premier Institutions</h4>
                               <ul className="tl-event-details-key-content-list phd-container">
                                    <li>Harvard Business School Publishing House</li>
                                    <li>CDAC</li>
                                    <li>IIT Kharagpur</li>
                                    <li>IIT Bombay</li>
                                    <li>COEP, Pune</li>
                                    <li>Indian Institute of Tropical Meteorology (IITM), Pune</li>
                                    <li>Institute of Management Technology (IMT), Ghaziabad</li>
                                    <li>KLE Technology</li>
                                    <li>Hon. Shri. Annasaheb Dange Arts, Commerce & Science College, Hatkanangale</li>
                                    <li>Heartfulness Institute</li>
                                </ul>
                            </div>
                        </div>


                        <div className="tl-event-details-left mt-30">
                        <div className="tl-event-details-key-content tl-event-details-area">
                            <h4>Collaborations with Industry</h4>
                               <ul className="tl-event-details-key-content-list phd-container">
                                    <li>Tata Technologies</li>
                                    <li>Infosys</li>
                                    <li>Kirloskar Oil Engines Ltd.</li>
                                    <li>PARI</li>
                                    <li>Star Air</li>
                                    <li>VOLVO</li>
                                    <li>Eicher</li>
                                    <li>Global Talent Track Private Limited</li>
                                </ul>
                            </div>
                        </div>

                      
                    </div>
                </div>
            </div>
        </div>
   )
}

export default IndustryCollaborations